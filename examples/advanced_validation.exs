...
user_with_address_schema =
  Sinter.Schema.define([
    ...
    {:addresses, {:array, address_schema}, [optional: true]},
    {:primary_address, address_schema, [optional: true]},
...
_tags_schema =
  Sinter.Schema.define([
    {:tags, {:array, :string}, [required: true, min_items: 1, max_items: 10]},
...
case Sinter.Validator.validate_many(user_with_address_schema, mixed_records) do
  ...
  {:error, error_map} ->
    IO.puts(
      "✓ Batch validation correctly identified errors at indices: #{inspect(Map.keys(error_map))}"
    )
...
complete_user_schema =
  Sinter.merge_schemas([base_entity, user_fields],
    title: "Complete User Schema"
  )

complete_product_schema =
  Sinter.merge_schemas([base_entity, product_fields],
...
IO.puts(
  "✓ Composed product validated: #{validated_product["title"]} ($#{validated_product["price"]})"
)
